...
// machine keeps reporting SmartCourse/OPCourse identity for whatever's actually resident, not
// the id requested. OperationStart's SmartCourse field isn't a live selector - the machine
// only ever runs whatever's been downloaded onto it. smart_course is a select instead: it
// shows what's resident and triggers a real download (see synthesizeSmartCourseDownload below
// and cloud/thinq1/http.ts's WasherCourseDownloadSvc) the moment a different one is picked.
...
// it set - it's just defining the course, not starting one. See
// synthesizeSmartCourseDownload() below, which passes initial:false.
...
// Every SmartCourse download uses this fixed APCourse sentinel: the machine always reports
// APCourse=10 whenever a SmartCourse is resident (see the header comment above), and all 16
// SmartCourse entries in modelJson carry APCourse:10.
const SMART_COURSE_AP_COURSE = 10

// Soil/SpinSpeed/WaterTemp/OPCourse for each SmartCourse, from modelJson's own
// SmartCourse[id].function array and OPCourse field. Only id 51 (Small Load) is verified
// against a real downloaded course; the rest are read from modelJson but unconfirmed. Lets
// synthesizeSmartCourseDownload() below build a real CourseDownload payload for any of these
// 16 entirely locally, with no LG dependency at all.
...
// The persistent-socket message that tells the device "a course is ready to download": the
// app sends this with a real LG-issued work-order id, and the device echoes it back verbatim
// as WasherCourseDownloadSvc's selectedCd. Sending it ourselves with our own id (just the
// SmartCourse's numeric id - the device doesn't validate the id's format, only relays it)
// triggers the exact same download flow, entirely locally.
