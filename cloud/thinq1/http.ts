...
function localCourseUrl(config: Config, courseId: string, modelName: string | undefined): string {
    ...
    // thinq1_https_port has no separate advertise setting (unlike https_port) - devices always
    // connect to the same port this server actually binds to.
    const port = config.thinq1_https_port.bind?.port ?? 46030
    return `https://${config.hostname}:${port}${COURSE_DOWNLOAD_PATH}?courseId=${encodeURIComponent(courseId)}${model}`
}
...
export function routes(
    ...
    router.post('/lgehadm/api/Rtos/WasherCourseDownloadSvc', async (req, res) => {
        ...
        // the persistent socket (InfoAlarm/CmdOpt:Course, base64 XML) - a lookup against LG's
        // real course-catalog backend, not raw course data. Real LG returns a downUrl at
        // aic.lgthinq.com (a different, non-rewritten host - the content fetch normally
        // happens directly against real LG, invisible to rethink), then the device hits
        // CourseDownloadCompleteSvc below to finish a real download.
        //
        // Answered locally when possible (synthesized from modelJson, or already cached from
        // a real capture below) so a real download never has to happen at all. Otherwise falls
        // through to proxying the lookup to real LG, capturing its downUrl's content for the
        // cache along the way.
        const deviceId = req.header('x-lgedm-deviceid')
        ...
        if (proxied && courseId && courseCache) {
            ...
            // locally-unsynthesizable course) - the device still gets LG's real response
            // verbatim below rather than a rewritten downUrl, since it's unconfirmed whether
            // LG's backend needs to see the device's own fetch to consider a download
            // confirmed. Not awaited: populating our own cache is unrelated to what the device
            // needs right now.
            if (realDownUrl) {
    ...
    router.post('/lgehadm/api/Rtos/CourseDownloadCompleteSvc', async (req, res) => {
        ...
        // that the download completed, not a data transfer itself.
        const deviceId = req.header('x-lgedm-deviceid')
    ...
    // - Logs every unknown endpoint's full method/path/body, so discovering a new one is a
    //   matter of watching the HTTPS log topic, not stumbling onto it.
    // - While actively bridged, proxies unknown endpoints to the real LG server and relays its
    //   real response back instead of a guessed generic ack - the same technique used for
    //   ContentsVerSvc/WasherCourseDownloadSvc, generalized so a future endpoint needs no
    //   individual whitelisting first.
